    ...
    <el-form :label-position="labelPosition" label-width="100px" :model="formPermission">
      ...
      <el-form-item label="拥有权限" class="perm-set">
        ...
        <el-checkbox-group v-model="checkedPerm" @change="handleCheckedCitiesChange">
                <el-checkbox v-for="item in permOptions" :label="item" :key="item">{{item}}</el-checkbox>
      </el-checkbox-group>
      </el-form-item>
    </el-form>
...
const checkOptions = ['通讯录管理', '工作台管理（该权限不受管理范围限制）', '企业设置（该权限不受管理范围限制）', '通知公告','工作台轮播图（该权限不受管理范围限制）'];
 ...
 export default {
   components:{
    ...
    handleSelectSubAdmin(){
          ...
          console.log(this.subAdminState)  
          //this.$message('努力提交中~');
                ...
                .then((res)=> {
                    ...
                    if(_data.code==0){
                      // console.log(_data)
                        ...
                        this.tableData.unshift(_data.data);
                    }else{
    ...
    handleClickModify(scope) {
      ...
      let __now=[];
      scope.row.ModelIDs.map((item)=>{
        __now.push(checkOptions[parseInt(item)-1])
     });
    ...
    handleCheckAllChange(val) {
         this.checkedPerm = val ? checkOptions : [];
  ...
  // 调整权限确定按钮
    handleChangePerConfirm(){
      let __modelids=[];
      this.checkedPerm.map((item)=>{
        if(checkOptions.indexOf(item)>-1){
          __modelids.push(checkOptions.indexOf(item)+1)
        }
      })
